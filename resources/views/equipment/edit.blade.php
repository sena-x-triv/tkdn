    ...
    <form action="{{ route('master.equipment.update', $equipment->id) }}" method="POST" class="space-y-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="number" name="tkdn" id="tkdn" value="{{ old('tkdn', $equipment->tkdn) }}" class="form-input w-full @error('tkdn') border-red-500 @enderror" min="0" max="100" step="0.01">
                @error('tkdn')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            <div>
                ...
                <input type="number" name="period" id="period" value="{{ old('period', $equipment->period) }}" class="form-input w-full @error('period') border-red-500 @enderror" min="1" required>
                @error('period')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            <div>
                ...
                <input type="number" name="price" id="price" value="{{ old('price', $equipment->price) }}" class="form-input w-full @error('price') border-red-500 @enderror" min="0" required>
                @error('price')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            <div class="md:col-span-2">
                ...
                <input type="text" name="description" id="description" value="{{ old('description', $equipment->description) }}" class="form-input w-full @error('description') border-red-500 @enderror">
                @error('description')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
